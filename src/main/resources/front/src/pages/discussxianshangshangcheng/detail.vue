	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/discussxianshangshangcheng?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">鐢ㄦ埛鍚?/div>
						<div class="text "  >{{detail.nickname}}</div>
					</div>
					<div class="item">
						<div class="lable">璇勮鍐呭</div>
						<div class="text">
							<span class="ql-snow ql-editor" v-html="detail.content"></span>
						</div>
					</div>
					<div class="item">
						<div class="lable">鍥炲鍐呭</div>
						<div class="text">
							<span class="ql-snow ql-editor" v-html="detail.reply"></span>
						</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('discussxianshangshangcheng','淇敼')" @click="editClick">淇敼</el-button>
						<el-button class="delBtn" v-if="btnAuth('discussxianshangshangcheng','鍒犻櫎')" @click="delClick">鍒犻櫎</el-button>
						<el-button class="editBtn" @click="discussClick()" type="warning" v-if="btnAuth('discussxianshangshangcheng','鏌ョ湅璇勮')">鏌ョ湅璇勮</el-button>
					</div>
				</div>
			</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 淇敼
			editClick(){
				this.$router.push(`/index/discussxianshangshangchengAdd?type=edit&&id=${this.detail.id}`);
			},
			// 鍒犻櫎
			async delClick(){
				await this.$confirm('鏄惁鍒犻櫎姝ょ嚎涓婂晢鍩庤瘎璁猴紵') .then(_ => {
					this.$http.post('discussxianshangshangcheng/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '鍒犻櫎鎴愬姛!',
