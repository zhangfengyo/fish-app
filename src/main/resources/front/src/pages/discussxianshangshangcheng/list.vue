		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('discussxianshangshangcheng','鏂板')" type="primary" @click="add('/index/discussxianshangshangchengAdd')">
			...
			<div class="list">
				<el-table class="tables" :stripe='false'
					...
					<el-table-column :resizable='true' :sortable='false'
						prop="nickname"
						label="鐢ㄦ埛鍚? show-overflow-tooltip>
					...
					<el-table-column :resizable='true' :sortable='false'
						prop="content"
						label="璇勮鍐呭" show-overflow-tooltip>
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="鎿嶄綔">
						<template slot-scope="scope">
							...
							<el-button class="table-del" type="success" @click.native="delClick(scope.row)" v-if="btnAuth('xianshangshangcheng','鍒犻櫎璇勮')">
								<span class="icon iconfont icon-shanchu6"></span>
								鍒犻櫎
							</el-button>
							<el-button class="table-btn1" type="success" @click.native="discussClick(scope.row)" v-if="btnAuth('discussxianshangshangcheng','鏌ョ湅璇勮')">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`discussxianshangshangcheng/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/discussxianshangshangchengDetail', query: params});
			},
			...
			discussClick (row){
				...
				this.$router.push({path: '/index/discussdiscussxianshangshangcheng', query: params});
			},
			editClick (row){
				this.$router.push(`/index/discussxianshangshangchengAdd?type=edit&&id=${row.id}`);
			},
			async delClick (row){
				await this.$confirm('鏄惁鍒犻櫎姝よ瘎璁猴紵') .then(_ => {
					this.$http.post('discussxianshangshangcheng/delete', [row.id]).then(async res => {
						if (res.data.code == 0) {
							await this.$http.get('xianshangshangcheng/info/' + row.refid).then(async rs=>{
								rs.data.data.discussnum--
								await this.$http.post('xianshangshangcheng/update',rs.data.data).then(rs2=>{})
							})
							this.$message({
								message: "鎿嶄綔鎴愬姛",
								type: "success",
			...
			discussistopChange(e,row){
				this.$http.post('discussxianshangshangcheng/update',row).then(res=>{})
			},
		}
	}
</script>
