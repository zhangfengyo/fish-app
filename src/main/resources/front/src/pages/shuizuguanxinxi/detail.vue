	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/shuizuguanxinxi?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">鍦洪缂栧彿</div>
						<div class="text "  >{{detail.changguanbianhao}}</div>
					</div>
					<div class="item">
						<div class="lable">鍦洪鍒嗙被</div>
						<div class="text "  >{{detail.changguanfenlei}}</div>
					</div>
					<div class="item">
						<div class="lable">鍦洪浣嶇疆</div>
						<div class="text "  >{{detail.changguanweizhi}}</div>
					</div>
					<div class="item">
						<div class="lable">鑱旂郴浜?/div>
						<div class="text "  >{{detail.lianxiren}}</div>
					</div>
					<div class="item">
						<div class="lable">鑱旂郴鐢佃瘽</div>
						<div class="text "  >{{detail.lianxidianhua}}</div>
					</div>
					<div class="item">
						<div class="lable">寮€鏀炬椂闂?/div>
						<div class="text "  >{{detail.kaifangshijian}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('shuizuguanxinxi','淇敼')" @click="editClick">淇敼</el-button>
						<el-button class="delBtn" v-if="btnAuth('shuizuguanxinxi','鍒犻櫎')" @click="delClick">鍒犻櫎</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('shuizuguanxinxi','绔犺妭绠＄悊')">绔犺妭绠＄悊</el-button>
					</div>
				</div>
			</div>
			...
			<el-tabs id="tabsBox" class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="鍦洪浠嬬粛" name="1">
					<div class="ql-snow ql-editor" v-html="detail.changguanjieshao"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						if(this.detail.changguantupian) {
							this.detailBanner = this.detail.changguantupian.split(",w").length>1?[this.detail.changguantupian]:this.detail.changguantupian.split(',');
						}
						this.$forceUpdate();
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('shuizuguanxinxi/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '鏀惰棌鎴愬姛!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'shuizuguanxinxi', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/shuizuguanxinxi', query: params});
				}
			},
			// 涓嬭浇
			download(file ){
			...
			// 淇敼
			editClick(){
				this.$router.push(`/index/shuizuguanxinxiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 鍒犻櫎
			async delClick(){
				await this.$confirm('鏄惁鍒犻櫎姝ゆ按鏃忛淇℃伅锛?) .then(_ => {
					this.$http.post('shuizuguanxinxi/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
