	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/pinzhongfenlei?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('pinzhongfenlei','淇敼')" @click="editClick">淇敼</el-button>
						<el-button class="delBtn" v-if="btnAuth('pinzhongfenlei','鍒犻櫎')" @click="delClick">鍒犻櫎</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('pinzhongfenlei','绔犺妭绠＄悊')">绔犺妭绠＄悊</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/pinzhongfenlei', query: params});
				}
			},
			// 涓嬭浇
			download(file ){
			...
			// 淇敼
			editClick(){
				this.$router.push(`/index/pinzhongfenleiAdd?type=edit&&id=${this.detail.id}`);
			},
			// 鍒犻櫎
			async delClick(){
				await this.$confirm('鏄惁鍒犻櫎姝ゅ搧绉嶅垎绫伙紵') .then(_ => {
					this.$http.post('pinzhongfenlei/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '鍒犻櫎鎴愬姛!',
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chapterpinzhongfenlei', query: params});
			},
			// 鑾峰彇uuid
			getUUID() {
