		...
		<el-form
			...
			<el-form-item class="add-item" label="鍦洪鍒嗙被" prop="changguanfenlei">
				<el-input v-model="ruleForm.changguanfenlei" 
					placeholder="鍦洪鍒嗙被" clearable :readonly="ro.changguanfenlei"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 澶氱骇鑱斿姩鍙傛暟
			async info() {
				await this.$http.get(`changguanfenlei/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`changguanfenlei/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '鎿嶄綔鎴愬姛',
									type: 'success',
