	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/siyangjiaoxue?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">楸肩梾娌荤枟</div>
						<div class="text "  >{{detail.yubingzhiliao}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('siyangjiaoxue','淇敼')" @click="editClick">淇敼</el-button>
						<el-button class="delBtn" v-if="btnAuth('siyangjiaoxue','鍒犻櫎')" @click="delClick">鍒犻櫎</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('siyangjiaoxue','绔犺妭绠＄悊')">绔犺妭绠＄悊</el-button>
					</div>
				</div>
			</div>
			...
			<video v-if="detail.jiaoxueshipin" ref="jiaoxueshipin" :src="baseUrl +  detail.jiaoxueshipin"  controls="controls" >
				鎮ㄧ殑娴忚鍣ㄤ笉鏀寔瑙嗛鎾斁
			</video>

			<el-tabs id="tabsBox" class="detail-tabs" v-model="activeName" type="border-card" v-if="tabsNum>0" >
				<el-tab-pane label="鏁欏璇︽儏" name="1">
					<div class="ql-snow ql-editor" v-html="detail.jiaoxuexiangqing"></div>
				</el-tab-pane>
			</el-tabs>
	...
	export default {
		...
		methods: {
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(async res => {
					if (res.data.code == 0) {
						...
						this.title = this.detail.jiaoxuebiaoti;
			...
			storeup(type) {
				if (type == 1 && !this.isStoreup) {
					...
					this.$http.post('storeup/add', this.storeupParams).then(res => {
						if (res.data.code == 0) {
							...
							this.$http.post('siyangjiaoxue/update', this.detail).then(res => {});
							this.$message({
								type: 'success',
								message: '鏀惰棌鎴愬姛!',
								duration: 1500,
				...
				if (type == -1 && this.isStoreup) {
					this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: '1', refid: this.detail.id, tablename: 'siyangjiaoxue', userid: Number(localStorage.getItem('frontUserid'))}}).then(res => {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/siyangjiaoxue', query: params});
				}
			},
			// 涓嬭浇
			download(file ){
			...
			// 淇敼
			editClick(){
				this.$router.push(`/index/siyangjiaoxueAdd?type=edit&&id=${this.detail.id}`);
			},
			// 鍒犻櫎
			async delClick(){
				await this.$confirm('鏄惁鍒犻櫎姝らゲ鍏绘暀瀛︼紵') .then(_ => {
					this.$http.post('siyangjiaoxue/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
			...
			allchapterClick (){
				...
				this.$router.push({path: '/index/chaptersiyangjiaoxue', query: params});
			},
			// 鑾峰彇uuid
			getUUID() {
