	...
	<div>
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'/'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/weiyutixing?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">鐢ㄦ埛璐﹀彿</div>
						<div class="text "  >{{detail.yonghuzhanghao}}</div>
					</div>
					<div class="item">
						<div class="lable">鐢ㄦ埛濮撳悕</div>
						<div class="text "  >{{detail.yonghuxingming}}</div>
					</div>
					<div class="item">
						<div class="lable">鎻愰啋鏃ユ湡</div>
						<div class="text "  >{{detail.tixingriqi}}</div>
					</div>
					<div class="item">
						<div class="lable">鎻愰啋鍐呭</div>
						<div class="text "  >{{detail.tixingneirong}}</div>
					</div>
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('weiyutixing','淇敼')" @click="editClick">淇敼</el-button>
						<el-button class="delBtn" v-if="btnAuth('weiyutixing','鍒犻櫎')" @click="delClick">鍒犻櫎</el-button>
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('weiyutixing','绔犺妭绠＄悊')">绔犺妭绠＄悊</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/weiyutixing', query: params});
				}
			},
			// 涓嬭浇
			download(file ){
			...
			// 淇敼
			editClick(){
				this.$router.push(`/index/weiyutixingAdd?type=edit&&id=${this.detail.id}`);
			},
			// 鍒犻櫎
			async delClick(){
				await this.$confirm('鏄惁鍒犻櫎姝ゅ杺楸兼彁閱掞紵') .then(_ => {
					this.$http.post('weiyutixing/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '鍒犻櫎鎴愬姛!',
