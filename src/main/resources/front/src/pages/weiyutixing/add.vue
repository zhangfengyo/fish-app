...
<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="鎻愰啋鏍囬" prop="tixingbiaoti">
				<el-input v-model="ruleForm.tixingbiaoti" 
					placeholder="鎻愰啋鏍囬" clearable :readonly="ro.tixingbiaoti"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="鎻愰啋鏃ユ湡" prop="tixingriqi">
				<el-date-picker
					:disabled="ro.tixingriqi"
			...
			<el-form-item class="add-item" label="鎻愰啋鍐呭" prop="tixingneirong">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.tixingneirong"
					placeholder="鎻愰啋鍐呭"
...
<script>
	export default {
		...
		methods: {
			...
			// 澶氱骇鑱斿姩鍙傛暟
			async info() {
				await this.$http.get(`weiyutixing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`weiyutixing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '鎿嶄綔鎴愬姛',
									type: 'success',
