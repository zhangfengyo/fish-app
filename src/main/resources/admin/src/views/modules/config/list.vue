		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0","margin":"0 0 5px","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto"}'>
					...
					<el-button class="add" v-if="isAuth('config/' + configStatus,'鏂板')&&configStatus==1" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						鏂板
					</el-button>
					<el-button class="del" v-if="isAuth('config/' + configStatus,'鍒犻櫎')&&configStatus==1" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px 0px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('config/' + configStatus,'鏌ョ湅')"
					:data="dataList"
					v-loading="dataListLoading"
					@selection-change="selectionChangeHandler">
					<el-table-column :resizable='true' type="selection" align="center" width="50"></el-table-column>
					<el-table-column :resizable='true' :sortable='true' label="搴忓彿" type="index" width="50" />
					<el-table-column :resizable='true' :sortable='true' prop="name" label="鍚嶇О">
						<template slot-scope="scope">
							{{ changeBgName(scope.row.name) }}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="value" width="200" label="鍥剧墖">
						<template slot-scope="scope">
					...
					<el-table-column width="300" label="鎿嶄綔">
						<template slot-scope="scope">
							<el-button class="view" v-if="isAuth('config/' + configStatus,'鏌ョ湅')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								鏌ョ湅
							</el-button>
							<el-button class="edit" v-if="isAuth('config/' + configStatus,'淇敼')" type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('config/' + configStatus,'鍒犻櫎')&&configStatus==1" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
	...
	export default {
		...
		watch: {
			'$route' (to, from) { //鐩戝惉璺敱鏄惁鍙樺寲
				if(this.$route.params.type){//鍒ゆ柇鐘舵€佹槸鍚︽湁鍊?
				//璋冩暟鎹?
		...
		methods: {
			...
			async deleteHandler(id ) {
				if((!id&&(this.dataList.length - this.dataListSelections.length)<3)||(id&&this.dataList.length<=3)) {
					this.$message.error('杞挱鍥句笉鑳藉皬浜庝笁涓?)
					return false
				}
