	...
	<div class="main-content" :style='{"width":"100%","padding":"20px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"0","boxShadow":"none","margin":"0 10px 5px 0","borderRadius":"0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","position":"relative"}' v-if="!forumChild">
					<div  :style='{"border":"1px solid #ddd","width":"auto","padding":"0 10px","margin":"0 5px 0px 0","borderRadius":"4px","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">甯栧瓙鏍囬</label>
						<el-input v-model="searchForm.title" placeholder="甯栧瓙鏍囬" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"0","boxShadow":"none","margin":"0 10px 5px 0","borderRadius":"0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","position":"relative"}' v-if="forumChild">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px 0 8px","outline":"none","margin":"0 0 0 3px","color":"#fff","borderRadius":"4px","background":"#2dc7a3","width":"auto","fontSize":"inherit","height":"36px"}' type="success" @click="search()">杩斿洖</el-button>
				</el-row>
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0","margin":"0 0 5px","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto"}'>
					...
					<el-button class="add" v-if="isAuth('forum','鏂板')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						鏂板
					</el-button>
					<el-button class="del" v-if="isAuth('forum','鍒犻櫎')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px 0px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true' v-if="forumChild" prop="content" label="甯栧瓙鍐呭">
						<template slot-scope="scope">
					...
					<el-table-column :resizable='true' :sortable='true' v-if="!forumChild" prop="istop" label="鏄惁缃《">
						<template slot-scope="scope">
							<el-switch
					...
					<el-table-column :resizable='true' :sortable='true' v-if="!forumChild" prop="toptime" label="缃《鏃堕棿">
						<template slot-scope="scope">
							{{scope.row.toptime}}
						</template>
					</el-table-column>
					<el-table-column  v-if="!forumChild"  :resizable='true' prop="cover" width="200" label="灏侀潰">
						<template slot-scope="scope">
					...
					<el-table-column :resizable='true' :sortable='true' v-if="!forumChild" prop="isanon" label="鏄惁鍖垮悕">
						<template slot-scope="scope">
							{{ scope.row.isanon==1?'鏄?:'鍚? }}
						</template>
					</el-table-column>
					<el-table-column width="300" label="鎿嶄綔">
						<template slot-scope="scope">
							<el-button class="edit" v-if="!forumChild &&  isAuth('forum','淇敼') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('forum','鍒犻櫎')" type="primary" @click="deleteHandler(scope.row.id)">
