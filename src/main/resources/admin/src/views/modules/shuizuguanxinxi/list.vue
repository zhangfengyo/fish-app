		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"0","boxShadow":"none","margin":"0 10px 5px 0","borderRadius":"0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","position":"relative"}' >
					<div  :style='{"border":"1px solid #ddd","width":"auto","padding":"0 10px","margin":"0 5px 0px 0","borderRadius":"4px","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">鍦洪鍚嶇О</label>
						<el-input v-model="searchForm.changguanmingcheng" placeholder="鍦洪鍚嶇О" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div  :style='{"border":"1px solid #ddd","width":"auto","padding":"0 10px","margin":"0 5px 0px 0","borderRadius":"4px","display":"flex"}' class="select" label="鍦洪鍒嗙被" prop="changguanfenlei">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">鍦洪鍒嗙被</label>
						<el-select clearable v-model="searchForm.changguanfenlei" placeholder="璇烽€夋嫨鍦洪鍒嗙被" >
							<el-option v-for="(item,index) in changguanfenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div  :style='{"border":"1px solid #ddd","width":"auto","padding":"0 10px","margin":"0 5px 0px 0","borderRadius":"4px","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"inherit","display":"inline-block","lineHeight":"36px","fontSize":"inherit","fontWeight":"500","height":"36px"}' class="item-label">鍦洪浣嶇疆</label>
						<el-input v-model="searchForm.changguanweizhi" placeholder="鍦洪浣嶇疆" @keydown.enter.native="search()" clearable></el-input>
					</div>
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0","margin":"0 0 5px","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto"}'>
					...
					<el-button class="add" v-if="isAuth('shuizuguanxinxi','鏂板')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						鏂板
					</el-button>
					<el-button class="del" v-if="isAuth('shuizuguanxinxi','鍒犻櫎')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px 0px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					:border='false'
					v-if="isAuth('shuizuguanxinxi','鏌ョ湅')"
					:data="dataList"
					...
					<el-table-column :resizable='true' :sortable='true'
												prop="changguanfenlei"
						label="鍦洪鍒嗙被">
						<template slot-scope="scope">
							{{scope.row.changguanfenlei}}
						</template>
					</el-table-column>
					<el-table-column  :resizable='true' prop="changguantupian" width="200" label="鍦洪鍥剧墖">
						<template slot-scope="scope">
							<div v-if="scope.row.changguantupian">
								<img v-if="scope.row.changguantupian.substring(0,4)=='http'&&scope.row.changguantupian.split(',w').length>1" :src="scope.row.changguantupian" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.changguantupian)">
								<img v-else-if="scope.row.changguantupian.substring(0,4)=='http'" :src="scope.row.changguantupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView(scope.row.changguantupian.split(',')[0])">
								<img v-else :src="$base.url+scope.row.changguantupian.split(',')[0]" width="100" height="100" style="object-fit: cover" @click="imgPreView($base.url+scope.row.changguantupian.split(',')[0])">
							</div>
							<div v-else>鏃犲浘鐗?/div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="鎿嶄綔">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('shuizuguanxinxi','鏌ョ湅')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								鏌ョ湅
							</el-button>
							<el-button class="edit" v-if=" isAuth('shuizuguanxinxi','淇敼') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('shuizuguanxinxi','鍒犻櫎')" type="primary" @click="deleteHandler(scope.row.id)">
	...
	export default {
		...
		methods: {
			...
			getDataList() {
				...
				if(this.searchForm.changguanfenlei!='' && this.searchForm.changguanfenlei!=undefined){
					params['changguanfenlei'] = this.searchForm.changguanfenlei
				}
