...
<template>
	<div class="addEdit-block">
		...
		>
			...
			<el-form-item label="璇勮鍐呭" prop="content" class="textBox">
				<span class="text ql-snow ql-editor" v-html="ruleForm.content"></span>
			</el-form-item>
			<el-form-item v-if="type!='info'"  label="鍥炲鍐呭" prop="reply" class="editorBox">
				<editor 
			...
			<el-form-item v-else-if="ruleForm.reply" label="鍥炲鍐呭" prop="reply" class="textBox">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async info(id) {
				await this.$http({
					url: `discussxianshangshangcheng/info/${id}`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `discussxianshangshangcheng/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discussxianshangshangchengCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
