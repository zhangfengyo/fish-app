	...
	<div class="main-content" :style='{"width":"100%","padding":"20px","fontSize":"14px","color":"#666","height":"auto"}'>
		...
		<template v-if="showFlag ">
			<el-form class="center-form-pv" :style='{"width":"100%","padding":"0","margin":"0 0 0px","flexWrap":"wrap","display":"flex"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"border":"0px solid #e5e7eb","padding":"0","boxShadow":"none","margin":"0 10px 5px 0","borderRadius":"0","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto","position":"relative"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px 0 8px","outline":"none","margin":"0 0 0 3px","color":"#fff","borderRadius":"4px","background":"#2dc7a3","width":"auto","fontSize":"inherit","height":"36px"}' type="success" @click="backClick()">杩斿洖</el-button>
				</el-row>
				...
				<el-row class="actions" :style='{"border":"0px solid #e5e7eb","padding":"0","margin":"0 0 5px","borderRadius":"0","flexWrap":"wrap","background":"#fff","display":"flex","width":"auto"}'>
					...
					<el-button class="add" v-if="isAuth('discussxianshangshangcheng','鏂板')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0 2px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
						鏂板
					</el-button>
					<el-button class="del" v-if="isAuth('discussxianshangshangcheng','鍒犻櫎')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"width":"100%","padding":"10px 0px 20px","boxShadow":"none","borderRadius":"0 0 4px 4px","background":"#fff"}'>
				<el-table class="tables"
					...
					<el-table-column :resizable='true' :sortable='true' prop="content" label="璇勮鍐呭">
						<template slot-scope="scope">
							<span class="ql-snow ql-editor" v-html="scope.row.content"></span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="reply" label="鍥炲鍐呭">
						<template slot-scope="scope">
							<span class="ql-snow ql-editor" v-html="scope.row.reply"></span>
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true' prop="istop" label="鏄惁缃《">
						<template slot-scope="scope">
							<el-switch
					...
					<el-table-column width="300" label="鎿嶄綔">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussxianshangshangcheng','淇敼') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai13" :style='{"margin":"0 0px","fontSize":"14px","color":"inherit","display":"none","height":"40px"}'></span>
								缂栬緫
							</el-button>

							<el-button class="view" v-if="isAuth('discussxianshangshangcheng','鏌ョ湅璇勮')" type="success" @click="disscussListHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('xianshangshangcheng','鍒犻櫎璇勮')" type="primary" @click="deleteHandler(scope.row.id ,scope.row.refid)">
