...
<template>
	<div class="addEdit-block">
		...
		>
			<template >
				<el-form-item class="input" v-if="type!='info'"  label="鍝佺鍒嗙被" prop="pinzhongfenlei" >
					<el-input v-model="ruleForm.pinzhongfenlei" placeholder="鍝佺鍒嗙被" clearable  :readonly="ro.pinzhongfenlei"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="鍝佺鍒嗙被" prop="pinzhongfenlei" >
					<el-input v-model="ruleForm.pinzhongfenlei" placeholder="鍝佺鍒嗙被" readonly></el-input>
				</el-form-item>
			</template>
	...
	export default {
		data() {
			return {
				...
				rules: {
					pinzhongfenlei: [
						{ required: true, message: '鍝佺鍒嗙被涓嶈兘涓虹┖', trigger: 'blur' },
					],
				},
			};
		},
			...
			async onSubmit() {
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `pinzhongfenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.pinzhongfenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
										}
									});
								} else {
